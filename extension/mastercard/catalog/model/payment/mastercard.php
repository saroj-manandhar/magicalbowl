...
class Mastercard extends \Opencart\System\Engine\Model {
    ...
    */
    ...
    public function getGatewayUri() {
        ...
        if ($gatewayUrl && substr($gatewayUrl, -1) !== '/') {
            $gatewayUrl .= '/';
        }
        return $gatewayUrl;
    }
    ...
    */
    ...
    public function apiRequest($method, $uri, $data = []) {
        ...
        $resolveIp = $this->getGatewayResolveIps($host);
        if ($resolveIp) {
        ...
        if ($httpCode >= 400) {
            $errResponse = json_decode($output, true);
            if (!empty($errResponse['error'])) {
                throw new \Exception(json_encode($errResponse['error']));
            } else {
                throw new \Exception("Gateway API HTTP Error (Status Code " . $httpCode . "): " . $output);
            }
        }
    ...
    private function getGatewayResolveIps($host) {
        $mappings = [
            'mtf.gateway.mastercard.com' => '216.119.222.23',
            'ap-gateway.mastercard.com' => '103.55.149.32',
            'eu-gateway.mastercard.com' => '185.175.205.180',
            'na-gateway.mastercard.com' => '216.119.218.80'
        ];
        return $mappings[$host] ?? null;
    }
