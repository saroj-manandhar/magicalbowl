...
class Mastercard extends \Opencart\System\Engine\Controller {
    ...
    public function index() {
        ...
        $is_valid = $this->isValidSession($integrationModel);
        $this->log->write("Mastercard checkout check - isValidSession: " . ($is_valid ? "true" : "false"));
        if (!$is_valid) {
            $this->log->write("Session details: " . json_encode([
                'order_id' => $this->session->data['order_id'] ?? null,
                'currency' => $this->session->data['currency'] ?? null,
                'shipping_address' => $this->session->data['shipping_address'] ?? null,
                'integrationModel' => $integrationModel
            ]));
            $customer = $this->getCustomer();
            if (empty($customer['email']) || strlen(trim($customer['email'])) < 3 || (empty($this->session->data['shipping_address']) && empty($this->session->data['payment_address']))) {
                return '<div class="alert alert-info"><i class="fa-solid fa-circle-info"></i> Please fill in your address and contact details first to proceed with Mastercard payment.</div>';
            }
        }
        ...
        if ($is_valid) {
            try {
                ...
                $this->log->write("Mastercard checkout session init exception: " . $e->getMessage());
                $err_data = json_decode($e->getMessage(), true);
                if (isset($err_data['explanation'])) {
                    $data['error_session'] = $err_data['explanation'] . ' (' . ($err_data['cause'] ?? '') . ')';
                } else {
            ...
            if (!empty($data['error_session'])) {
                $view = '<div class="alert alert-danger"><i class="fa-solid fa-circle-exclamation"></i> <strong>Mastercard Gateway Error:</strong> ' . htmlspecialchars($data['error_session']) . '</div>';
            } elseif ($integrationModel === 'hostedcheckout') {
    ...
    private function isValidSession($integrationModel) {
        $customer = $this->getCustomer();
        $hasCustomerEmail = !empty($customer['email']) && strlen(trim($customer['email'])) >= 3;
        $hasOrderId = !empty($this->session->data['order_id']);
        $hasCurrency = !empty($this->session->data['currency']);
        $hasAddress = !empty($this->session->data['shipping_address']) || !empty($this->session->data['payment_address']);
    ...
    public function hasCheckoutSession() {
        ...
        $notificationUrl = $this->url->link('extension/mastercard/payment/mastercard.callback', '', true);
        $parsedUrl = parse_url($notificationUrl);
        if (isset($parsedUrl['port'])) {
            $notificationUrl = str_replace(':' . $parsedUrl['port'], '', $notificationUrl);
        }
        $requestData['order']['notificationUrl'] = $notificationUrl;
        if (!empty($this->getShippingAddress())) {
            $requestData = array_merge($requestData, ['shipping' => $this->getShippingAddress()]);
        }
        $requestData = $this->convertToTxnCurrency($requestData);
        unset($this->session->data['HostedCheckout_sessionId']);
        ...
        try {
            ...
            $this->log->write("Mastercard API connection error: " . $e->getMessage() . " | URI: " . $uri . " | Payload: " . json_encode($requestData));
            throw $e;
        }
    ...
    /**
     * Convert request order details and amounts to a transaction currency supported by the merchant.
     *
     * @param array $requestData
     * @return array
     */
    protected function convertToTxnCurrency(array $requestData): array {
        $supportedCurrencies = ['NPR', 'USD', 'CNY'];
        $sessionCurrency = $this->session->data['currency'] ?? 'USD';

        if (in_array($sessionCurrency, $supportedCurrencies)) {
            $txnCurrency = $sessionCurrency;
        } else {
            $txnCurrency = 'USD';
        }

        $defaultCurrency = $this->config->get('config_currency') ?? 'USD';

        $requestData['order']['currency'] = $txnCurrency;

        if ($defaultCurrency === $txnCurrency) {
            return $requestData;
        }

        if (isset($requestData['order']['amount'])) {
            $requestData['order']['amount'] = round($this->currency->convert($requestData['order']['amount'], $defaultCurrency, $txnCurrency), 2);
        }

        if (isset($requestData['order']['itemAmount'])) {
            $requestData['order']['itemAmount'] = round($this->currency->convert($requestData['order']['itemAmount'], $defaultCurrency, $txnCurrency), 2);
        }

        if (isset($requestData['order']['shippingAndHandlingAmount'])) {
            $requestData['order']['shippingAndHandlingAmount'] = round($this->currency->convert($requestData['order']['shippingAndHandlingAmount'], $defaultCurrency, $txnCurrency), 2);
        }

        if (isset($requestData['order']['taxAmount'])) {
            $requestData['order']['taxAmount'] = round($this->currency->convert($requestData['order']['taxAmount'], $defaultCurrency, $txnCurrency), 2);
        }

        if (isset($requestData['order']['item']) && is_array($requestData['order']['item'])) {
            foreach ($requestData['order']['item'] as &$item) {
                if (isset($item['unitPrice'])) {
                    $item['unitPrice'] = round($this->currency->convert($item['unitPrice'], $defaultCurrency, $txnCurrency), 2);
                }
            }
        }

        if (isset($requestData['order']['tax']) && is_array($requestData['order']['tax'])) {
            foreach ($requestData['order']['tax'] as &$tax) {
                if (isset($tax['amount'])) {
                    $tax['amount'] = round($this->currency->convert($tax['amount'], $defaultCurrency, $txnCurrency), 2);
                }
            }
        }
    ...
    protected function getCustomer() {
        ...
        if (empty($customerData['email']) && !empty($this->session->data['customer']['email'])) {
            $customerData['firstName'] = substr($this->session->data['customer']['firstname'] ?? '', 0, 50);
            $customerData['lastName']  = substr($this->session->data['customer']['lastname'] ?? '', 0, 50);
            $customerData['email']     = $this->session->data['customer']['email'];
        }

        if (empty($customerData['email']) && !empty($this->session->data['order_id'])) {
            $orderId = (int)$this->session->data['order_id'];
