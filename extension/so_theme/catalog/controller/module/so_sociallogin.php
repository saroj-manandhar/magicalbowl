...
class SoSociallogin extends \Opencart\System\Engine\Controller {	
	public function index() {
		...
		if (isset($setting) && $setting['so_sociallogin_enable'] && $this->config->get('so_sociallogin_enable')) {
			...
			$this->load->language('extension/so_theme/module/so_sociallogin','',$this->config->get('config_language_catalog'));
			...
			require_once (DIR_EXTENSION.'so_theme/system/library/so_social/Facebook/autoload.php');
			
			// Google Libery file inculde
			require_once DIR_EXTENSION.'so_theme/system/library/so_social/src/Google_Client.php';
			require_once DIR_EXTENSION.'so_theme/system/library/so_social/src/contrib/Google_Oauth2Service.php';
			...
			$data['fblink'] = $helper->getLoginUrl($this->url->link('extension/so_theme/module/so_sociallogin.FacebookLogin', '', 'SSL'), array('public_profile','email'));
            /* Facebook  Login link code */
			
			
			/* Twitter Login */						
			$data['twitlink'] =  $this->url->link('extension/so_theme/module/so_sociallogin.TwitterLogin', '', 'SSL');
				
			/* Linkedin Login */
			$data['linkdinlink'] = $this->url->link('extension/so_theme/module/so_sociallogin.LinkedinLogin', '', 'SSL');
			 /* Linkedin Login */
            ...
            $gClient->setRedirectUri($this->url->link('extension/so_theme/module/so_sociallogin.GoogleLogin', '', 'SSL'));
			...
			{
				return $this->load->view('extension/so_theme/module/so_sociallogin/default', $data);
		    }
		}
	}
	...
	public function FacebookLogin() {
		...
		else
		{
			...
			$location = $this->url->link('account/account', 'language=' . $this->config->get('config_language') . '&customer_token=' . $this->session->data['customer_token'], true);
		}
		...
		try {
		  	$accessToken = $helper->getAccessToken($site_url.'index.php?route=extension/so_theme/module/so_sociallogin|FacebookLogin');
		} catch(Facebook\Exceptions\FacebookResponseException $e) {
		  	echo 'Graph returned an error: ' . $e->getMessage();
		  	exit;
		} catch(Facebook\Exceptions\FacebookSDKException $e) {
	...
	public function TwitterLogin() {
		...
		require_once (DIR_EXTENSION.'so_sociallogin/system/library/so_social/twitter/twitteroauth.php');
		
		//Fresh authentication
		$connection = new \TwitterOAuth($twitapikey, $twitsecretapi);
		$request_token = $connection->getRequestToken($this->url->link('extension/so_theme/module/so_sociallogin/TwitterToken', '', 'SSL'));
	...
	public function TwitterToken() {
		...
		require_once (DIR_EXTENSION.'so_theme/system/library/so_social/twitter/twitteroauth.php');
	...
	public function LinkedinLogin() {
		...
		require_once (DIR_EXTENSION.'so_theme/system/library/so_social/linkedIn/http.php');
		require_once (DIR_EXTENSION.'so_theme/system/library/so_social/linkedIn/oauth_client.php');
		
		$client = new \oauth_client_class;
		$client->debug = false;
		$client->debug_http = true;
		$client->redirect_uri = $this->url->link('extension/so_theme/module/so_sociallogin/LinkedinLogin', '', 'SSL');
