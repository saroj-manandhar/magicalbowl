...
class SoCategories extends \Opencart\System\Engine\Controller {	
	public function index($setting) {
		$this->load->language('extension/so_theme/module/so_categories','',$this->config->get('config_language_catalog'));
		...
		$folder_so_deal = DIR_TEMPLATE.$this->config->get('theme_default_directory').'/template/extension/so_theme/module/so_categories/';
		...
		foreach($_catids as $category_id){
			...
			$title_maxlength = (($setting['cat_title_maxcharacs'] != 0 && strlen($category_info['name']) > $setting['cat_title_maxcharacs']) ? substr(strip_tags(html_entity_decode($category_info['name'], ENT_QUOTES, 'UTF-8')), 0, $setting['cat_title_maxcharacs']) . '..' : $category_info['name']);
			if(isset($category_info['name']))
			{
				$data['list'][] = array(
					'title' 	=> html_entity_decode($category_info['name'], ENT_QUOTES, 'UTF-8'),
					'all_product' => $this->model_catalog_product->getTotalProducts(array('filter_category_id'=>$category_id)),
		...
		if (!class_exists('Cache_Lite'))
		    require_once (DIR_EXTENSION . 'so_theme/system/library/so/categories/Cache_Lite/Lite.php');
		...
		if ($use_cache){
			...
			if (!$_data) {
				$_data = $this->load->view('extension/so_theme/module/so_categories/default', $data);
			...
			return $this->load->view('extension/so_theme/module/so_categories/default', $data);
		}
		
	}
	public function getCategoryson($category_id, $setting){
		$checkCategory = $this->model_extension_so_theme_module_so_categories->checkCategory($category_id);
		$categoryss = array();
		if(isset($checkCategory) && $checkCategory[0]['status'] == 1 && $checkCategory != null && $setting['child_category'] ==1){
			...
			$categoryss = $this->model_extension_so_theme_module_so_categories->getCategories_son_categories($filter_data);
		}
		return $categoryss;
	}
