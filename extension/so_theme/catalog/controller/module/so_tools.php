...
class SoTools extends \Opencart\System\Engine\Controller {	
	...
	public function index($setting) {
		...
		static $module = 0;
		$this->load->language('extension/so_theme/module/so_tools','',$this->config->get('config_language_catalog'));
			...
			foreach($children as $child) {
				...
				foreach($subchildren as $subchild) {
					$subchildren_data[] = array(
						...
						'name' => $subchild['name'],
						'href' => $this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&path=' . $category['category_id'] . '_' . $child['category_id'] . '_' . $subchild['category_id'])
					);
				}
		...
		$data['link_order']		= $this->url->link('account/order', 'language=' . $this->config->get('config_language'));
		$data['link_download']	= $this->url->link('account/download', 'language=' . $this->config->get('config_language'));
		$data['link_register']	= $this->url->link('account/register', 'language=' . $this->config->get('config_language'));
		$data['link_account']	= $this->url->link('account/account', 'language=' . $this->config->get('config_language'));
		$data['link_cart']		= $this->url->link('checkout/cart', 'language=' . $this->config->get('config_language'));
		$data['link_login']		= $this->url->link('account/login', 'language=' . $this->config->get('config_language'));

		$this->document->addStyle('extension/so_theme/catalog/view/javascript/so_tools/css/style.css');		
		$data['tool_js'] = 'extension/so_theme/catalog/view/javascript/so_tools/js/script.js';

		// Currency
		$this->load->language('common/currency','',$this->config->get('config_language_catalog'));
		...
		// Language
		$this->load->language('common/language','',$this->config->get('config_language_catalog'));
		$data['text_language'] = $this->language->get('text_language');
		$data['action_language'] = $this->url->link('common/language/language', '', $this->request->server['HTTPS']);

		$data['code_language'] = $this->config->get('config_language');
		...
		// Shopping cart
		$this->load->language('common/cart','',$this->config->get('config_language_catalog'));
		// Totals
			...
			foreach ($this->session->data['sorecentproduct'] as $product_id) {
				...
				if (isset($product['product_id']) && !empty($product['product_id'])) {
					...
					$data['recent_products'][] = array(
						...
						'product_image'		=> $image,
						'product_price'     => $price,
						'product_discount'   => $special,
						'product_tax'       => $tax,
						'product_discount' 	=> $discount,
						'product_new' 		=> $productNew,
						'product_href'     	=> $this->url->link('product/product', 'language=' . $this->config->get('config_language') . '&product_id=' . $product['product_id'])
					);
		...
		return $this->load->view('extension/so_theme/module/so_tools/layout', $data);
	}
	

	public function remove_cart() {
		$this->load->language('checkout/cart','',$this->config->get('config_language_catalog'));
	...
	public function infocart() {
		...
		return $this->load->view('extension/so_theme/module/so_tools/infocart', $data);
	}
}