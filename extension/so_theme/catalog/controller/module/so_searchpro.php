...
class SoSearchpro extends \Opencart\System\Engine\Controller {	
	private $data = array();
	...
	public function index( $setting ) { 
		...
		if (!class_exists('Cache_Lite'))
			require_once (DIR_EXTENSION . 'so_theme/system/library/so/searchpro/Cache_Lite/Lite.php');
		...
		if ($use_cache){
			...
			if (!$_data) {
				...
				$_data = $this->load->view('extension/so_theme/module/so_searchpro/'.$data['store_layout'], $data);
			...
			$data = $this->readData($setting);
			return $this->load->view('extension/so_theme/module/so_searchpro/'.$data['store_layout'], $data);
		}
	...
	public function readData($setting) {
		...
		static $module = 0;
		$this->load->language('extension/so_theme/module/so_searchpro','',$this->config->get('config_language_catalog'));
		$this->load->model('tool/image');
		$this->load->model('catalog/category');
		$this->load->model('extension/so_theme/module/so_searchpro');
		/*Entry */
		...
		$this->data['showimage'] = isset($setting['showimage']) ? $setting['showimage'] : '0';
		$this->data['showprice'] = isset($setting['showprice']) ? $setting['showprice'] : '0';		
		$this->data['showcategory'] = isset($setting['showcategory']) ? $setting['showcategory']:'0';
		$this->data['show_brand'] = isset($setting['show_brand']) ? $setting['show_brand']:'0';
		$this->data['show_key'] = isset($setting['show_key']) ? $setting['show_key']:'0';
		...
		if (isset($setting['module_description'][$this->config->get('config_language_id')])) {
			$this->data['head_name'] = html_entity_decode(isset($setting['module_description'][$this->config->get('config_language_id')]['head_name'])?$setting['module_description'][$this->config->get('config_language_id')]['head_name']:'', ENT_QUOTES, 'UTF-8');
			...
			$this->data['head_name']  = isset($setting['head_name'])?$setting['head_name']:'';
		...
		if($this->data['show_brand'] && $this->data['show_brand'] == 1){
			$this->load->model('catalog/manufacturer');
			$filter_brand = array(
				'sort'	=> 'sort_order'
			);
			$this->data['brands'] = $this->model_catalog_manufacturer->getManufacturers($filter_brand);
		}
	...
	public function getProducts($setting)
	{
		...
		$this->load->language('extension/module/so_searchpro','',$this->config->get('config_language_catalog'));
		
		$products_arr = $this->model_extension_so_theme_module_so_searchpro->getProducts($filter_data);
		
		foreach($products_arr as $product_info){
			$name = $product_info['name'];
			$name_maxlength = ((strlen($product_info['name']) > 25)  ? substr(strip_tags(html_entity_decode($product_info['name'], ENT_QUOTES, 'UTF-8')), 0, 25) . '..' : $product_info['name']);
			$data['list_products'][] = array(
	...
	public function autocomplete() {
		...
		$this->load->language('product/category','',$this->config->get('config_language_catalog'));
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_category_id'])) {
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					...
					'model'      => $result['model'],
					'image'		 => $image,
					'link'		 => $this->url->link('product/product', 'language=' . $this->config->get('config_language') . '&path='.$path.'&product_id='.$result['product_id']),
					'special'	 => $special,
					'tax'		 => $tax,
					'price'      => ($result['model']<0)?$price:'',
