<?php
class ControllerExtensionAmpCategory extends Controller {
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'amp_cart'        => $this->url->link('product/product', 'product_id=' . $result['product_id']).'&addcart=true'
			...
			*/
			...
			$data['header'] = $this->load->controller('extension/amp/header');
			$data['footer'] = $this->load->controller('extension/amp/footer');

			$this->response->setOutput($this->load->view('amp/category', $data));
		} else {
			$url = '';
