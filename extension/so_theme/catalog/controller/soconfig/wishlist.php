...
class Wishlist extends \Opencart\System\Engine\Controller {
	public function index() {
		...
		$this->load->language('account/wishlist','',$this->config->get('config_language_catalog'));
		...
		if (isset($this->request->get['remove'])) {
			// Remove Wishlist
			$this->model_account_wishlist->deleteWishlist($this->customer->getId(), (int)$this->request->get['remove']);
	...
	public function add() {
		...
		$this->load->language('extension/so_theme/soconfig/wishlist','',$this->config->get('config_language_catalog'));
		$json = array();
		...
		if ($product_info) {
			if (!isset($this->session->data['wishlist'])) $this->session->data['wishlist'] = array();
			if (!in_array($this->request->post['product_id'], $this->session->data['wishlist'])) {
				...
				if ($this->customer->isLogged()) {
					// Edit customers cart
					$this->load->model('account/wishlist');
					$this->model_account_wishlist->addWishlist($this->customer->getId(), (int)$this->request->post['product_id']);
				...
				$json['info'] = $this->language->get('text_exists');
			}
		}
