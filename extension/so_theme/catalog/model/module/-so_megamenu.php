...
class SoMegamenu extends \Opencart\System\Engine\Model {
    ...
    public function getMenu($module_id) {
		...
		$sql = "SELECT c.category_id, c.parent_id, c.image, cd.name, c.category_id as id 
			...
			if($row['content'] != null) {
				...
				if(isset($content['manufacture']['id']) && $content['manufacture']['id']) {
					...
					foreach($content['manufacture']['id'] as $key=>$id_manufacture){
						...
						$manufacture['link'] = $this->url->link('product/manufacturer/info', 'language=' . $this->config->get('config_language_id') . '&manufacturer_id=' . $id_manufacture);
					...
					if($results){
						foreach($results as $key=>$result){
							...
							$productlist['products'][] = array(
								...
								'rating'      => $result['rating'],
								'href'        => $this->url->link('product/product', 'language=' . $this->config->get('config_language_id') . '&product_id=' . $result['product_id'])
							);
						}
					}
				}
				...
				if(isset($content['subcategory'])  && isset($content['subcategory']['category'])) {
					...
					if($array_categories){
						foreach($array_categories as $id_category){
							...
							$sub['href']  = $this->url->link('product/category', 'language=' . $this->config->get('config_language_id') . '&path=' . $id_category);
								...
								foreach ($child_categories as &$category) {
									...
									foreach ($children as $key=>$child) {
										...
										$children_data[$key] = array(
											...
											'href'  => $this->url->link('product/category', 'language=' . $this->config->get('config_language_id') . '&path=' . $category['category_id'] . '_' . $child['category_id']),
										...
										if($children2){
											foreach ($children2 as $child2){
												...
												$children_data[$key]['children'][] = array(
													...
													'href'  => $this->url->link('product/category', 'language=' . $this->config->get('config_language_id') . '&path=' . $child['category_id'] . '_' . $child2['category_id']),
				...
				if(isset($content['product']['id']) && !empty($content['product']['id'])) {			
					...
					if(is_array($product)) {
						$product_link = $this->url->link('product/product', 'language=' . $this->config->get('config_language_id') . '&product_id=' . $content['product']['id']);
						if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
					...
					if(isset($array['categories'][$i]['id'])) {
						...
						if(isset($info_category['category_id'])) {
							...
							if(is_array($info_category)) {
								...
								$link = $this->url->link('product/category', 'language=' . $this->config->get('config_language_id') . '&path=' . $path . $info_category['category_id']);	
