<?php
namespace Opencart\Admin\Model\Extension\SoTheme\Module;
class SoMegaMenu extends \Opencart\System\Engine\Model {	
    private $errors = array();
    public function generate_nestable_list($lang_id) {
		$chk = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "mega_menu` WHERE `Field` = 'id_old'");

		if (!$chk->num_rows) {

		$this->db->query("ALTER TABLE `" . DB_PREFIX . "mega_menu` ADD COLUMN `id_old` int(11) NOT NULL");
        ...
        if (!isset($this->request->get['module_id'])) {
            $action = $this->url->link('extension/so_theme/module/so_megamenu', 'user_token=' . $this->session->data['user_token'], 'SSL');
        } else {
            $action = $this->url->link('extension/so_theme/module/so_megamenu', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
        }
        ...
        foreach ($query->rows as $row) {
            ...
            $json = json_decode($row['name'], true);
			if(empty($json)) {
            ...
            $output .= '<a data-toggle="tooltip" title="Delete" href="'.$action.'&delete='.$row['id'].'" onclick="return confirm(\'Are you sure you want to delete?\')" class="fa fa-trash"></a><a data-toggle="tooltip" title="edit" href="'.$action.'&edit='.$row['id'].'" class="fa fa-pencil-alt"></a>';
    ...
    public function menu_showNested($parentID, $lang_id) {
        ...
        if (count($query->rows) > 0) {
            ...
            foreach ($query->rows as $row) {
                ...
                $output .= '<a data-toggle="tooltip" title="Duplicate" href="'.$action.'&duplicate='.$row['id'].'"  class="fa fa-plus"></a>';
                $output .= '<a data-toggle="tooltip" title="Change Status" href="'.$action.'&changestatus='.$row['id'].'" class="'.$class.'" ></a>';
                $output .= '<a  data-toggle="tooltip" title="Delete" href="'.$action.'&delete='.$row['id'].'" onclick="return confirm(\'Are you sure you want to delete?\')" class="fa fa-trash"></a>';
                $output .= "<a data-toggle='tooltip' title='edit'  href='".$action."&edit=".$row['id']."' class='fa fa-pencil-alt'></a><div class='dd-handle'>{$name} (ID: {$row['id']})</div>\n";
    ...
    public function addMenu($data) {
	    ...
	    if(isset($data['rang'])) {			
			if($data['parent_id'] === 0 ) {
                ...
                $this->db->query("INSERT INTO " . DB_PREFIX . "mega_menu SET id_old = '" . $this->request->get['duplicate'] .date('%H%M') . "' , name = '" . $this->db->escape(json_encode($data['name'])) . "',label_item = '".$data['label_item']."',icon_font = '".$data['icon_font']."',class_menu = '".$data['class_menu']."',  description = '" . $this->db->escape(json_encode($data['description'])) . "', icon = '" . $data['icon'] . "', parent_id = '". $data['parent_id'] ."', type_link = '" . $data['type_link'] . "', module_id = '" . $module_id . "', link = '" . $data['link'] . "', new_window = '" . $data['new_window'] . "', status = '" . $data['status'] . "', position = '" . $data['position'] . "', submenu_width = '" . $data['submenu_width'] . "', submenu_type = '" . $data['display_submenu'] . "', rang='".$data['rang']."', content_width='" . $data['content_width'] . "', content_type='" . $data['content_type'] . "',  content='" . $this->db->escape(json_encode($data['content'])) . "'");	
			}
			else {
                $this->db->query("INSERT INTO " . DB_PREFIX . "mega_menu SET name = '" . $this->db->escape(json_encode($data['name'])) . "',label_item = '".$data['label_item']."',icon_font = '".$data['icon_font']."',class_menu = '".$data['class_menu']."',  description = '" . $this->db->escape(json_encode($data['description'])) . "', icon = '" . $data['icon'] . "', parent_id = '". $data['parent_id'] .date('%H%M') . "', type_link = '" . $data['type_link'] . "', module_id = '" . $module_id . "', link = '" . $data['link'] . "', new_window = '" . $data['new_window'] . "', status = '" . $data['status'] . "', position = '" . $data['position'] . "', submenu_width = '" . $data['submenu_width'] . "', submenu_type = '" . $data['display_submenu'] . "', rang='".$data['rang']."', content_width='" . $data['content_width'] . "', content_type='" . $data['content_type'] . "',  content='" . $this->db->escape(json_encode($data['content'])) . "'");
			}
			
		}else {
                $this->db->query("INSERT INTO " . DB_PREFIX . "mega_menu SET name = '" . $this->db->escape(json_encode($data['name'])) . "',label_item = '".$data['label_item']."',icon_font = '".$data['icon_font']."',class_menu = '".$data['class_menu']."',  description = '" . $this->db->escape(json_encode($data['description'])) . "', icon = '" . $data['icon'] . "', parent_id = '". $data['parent_id'] ."', type_link = '" . $data['type_link'] . "', module_id = '" . $module_id . "', link = '" . $data['link'] . "', new_window = '" . $data['new_window'] . "', status = '" . $data['status'] . "', position = '" . $data['position'] . "', submenu_width = '" . $data['submenu_width'] . "', submenu_type = '" . $data['display_submenu'] . "', rang='1000', content_width='" . $data['content_width'] . "', content_type='" . $data['content_type'] . "', content='" . $this->db->escape(json_encode($data['content'])) . "'");
    ...
    public function saveMenu($data) {		
		...
		$module_id = (isset($this->request->get['module_id']) && $this->request->get['module_id']) ? $this->request->get['module_id'] : 0;
        $data['content']['categories']['categories'] = json_decode(html_entity_decode($data['content']['categories']['categories']), true);
        $this->db->query("UPDATE " . DB_PREFIX . "mega_menu SET name = '" . $this->db->escape(json_encode($data['name'])) . "', label_item = '".$data['label_item']."',icon_font = '".$data['icon_font']."',class_menu = '".$data['class_menu']."', description = '" . $this->db->escape(json_encode($data['description'])) . "', icon = '" . $data['icon'] ."',type_link = '" . $data['type_link'] . "', module_id = '" . $module_id . "', link = '" . $data['link'] ."', new_window = '" . $data['new_window'] ."', status = '" . $data['status'] ."', position = '" . $data['position'] ."', submenu_width = '" . $data['submenu_width'] ."', submenu_type = '" . $data['display_submenu'] ."', content_width = '" . $data['content_width'] ."', content_type = '" . $data['content_type'] ."', content = '" . $this->db->escape(json_encode($data['content'])) . "' WHERE id = '" . $data['id'] . "'");
    }
    ...
    public function getMenu($id) {
        ...
        if(count($query->rows) > 0) {
            ...
            foreach ($query->rows as $result) {
				$check = json_decode($result['content'], true);
				if(empty($check)) {
					$check = unserialize($result['content']);
				}
				if(gettype($check) == "string"){
					$check = json_decode(json_decode($result['content'], true), true);
				}
				
			
                $data = array(
                    'name' => !empty(@json_decode($result['name'], true))?@json_decode($result['name'], true):unserialize($result['name']),
                    'description' => !empty(@json_decode($result['description'], true))?@json_decode($result['description'], true):unserialize($result['description']),
                    'icon' => $result['icon'],
    ...
    public function getCategories($array = array()) {
        ...
        if(is_array($array) && !empty($array[0]) && count($array)>0) {
	...
	public function duplicateModule($module_id,$import_module){
		...
		if($parent_menu){
			foreach ($parent_menu as $menu) {
				$dane = $this->model_extension_so_theme_module_so_megamenu->getMenu(intval($menu['id']));
				$dane['module_id'] = $module_id;
				$id_parent_add = $this->model_extension_so_theme_module_so_megamenu->addMenu($dane);
				$subcategories = $this->model_extension_so_theme_module_so_megamenu->getSubMenu(intval($menu['id']));
				if($subcategories){
				foreach ($subcategories as $result) {
					$data = array(
							...
							'list_categories' => (isset($result['content']['categories']['categories']) && $result['content']['categories']['categories']) ? $this->model_extension_so_theme_module_so_megamenu->getCategories(json_decode($result['content']['categories']['categories'], true)) : ''
						);
						$this->model_extension_so_theme_module_so_megamenu->addMenu($data);
					}
				}
			}
		}
	...
	public function duplicateMenu($id_duplicate){
		$dane = $this->model_extension_so_theme_module_so_megamenu->getMenu(intval($id_duplicate));
		$id_parent_add = $this->model_extension_so_theme_module_so_megamenu->addMenu($dane);
		$subcategories = $this->model_extension_so_theme_module_so_megamenu->getSubMenu(intval($id_duplicate));
		if($subcategories){
			foreach ($subcategories as $result) {
				$data = array(
					'parent_id' => $id_parent_add,
