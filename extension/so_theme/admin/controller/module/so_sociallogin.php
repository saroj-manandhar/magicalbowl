...
class SoSociallogin extends \Opencart\System\Engine\Controller {	
	...
	public function index() {
		$this->load->language('extension/so_theme/module/so_sociallogin','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/so_theme/module/so_sociallogin', 'user_token=' . $this->session->data['user_token'] . '&module_id='.$module_id, 'SSL'));
			}elseif($action == "save_new"){
				$this->response->redirect($this->url->link('extension/so_theme/module/so_sociallogin', 'user_token=' . $this->session->data['user_token'], 'SSL'));
			}else{
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_sociallogin', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_sociallogin', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);			
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/so_theme/module/so_sociallogin', 'user_token=' . $this->session->data['user_token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/so_theme/module/so_sociallogin', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_sociallogin', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_sociallogin')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		$params	= $this->request->post['so_sociallogin'];

		if ((strlen($params['so_sociallogin_name']) < 3) || (strlen($params['so_sociallogin_name']) > 64)) {
	...
	function install() {
		...
		$this->model_setting_module->addModule('so_theme.so_sociallogin', $data_module);
	}
	...
	function uninstall() {
		...
		$this->model_setting_module->deleteModulesByCode('so_theme.so_sociallogin');
		
		$this->load->model('setting/event');
		$this->model_setting_event->deleteEventByCode('so_sociallogin');			
	}
	...
	private function setupEvent() {
		...
		$this->removeEvent();
		$this->model_extension_so_theme_module_soconfig_setting->addEvent('so_sociallogin','', "catalog/view/account/login/before", "extension/so_theme/event/so_sociallogin.so_login_before" , 1 , 1);	
		$this->model_extension_so_theme_module_soconfig_setting->addEvent('so_sociallogin','', "catalog/view/account/login/after", "extension/so_theme/event/so_sociallogin.so_login_after" , 1 , 1);	
        $this->model_extension_so_theme_module_soconfig_setting->addEvent('so_sociallogin','', "catalog/view/common/header/before", "extension/so_theme/event/so_sociallogin.so_sociallogin_header_before" , 1 , 1);
        $this->model_extension_so_theme_module_soconfig_setting->addEvent('so_sociallogin','', "catalog/view/common/header/after", "extension/so_theme/event/so_sociallogin.so_sociallogin_header_after" , 1 , 1);			
	}
