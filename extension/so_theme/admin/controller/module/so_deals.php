...
class SoDeals extends \Opencart\System\Engine\Controller {	
	...
	// Load language
	$this->load->language('extension/so_theme/module/so_deals','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
	...
	$this->load->model('extension/so_theme/module/so_deals');
	...
	if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
		...
		$this->response->redirect($this->url->link('extension/so_theme/module/so_deals', 'user_token=' . $this->session->data['user_token'], 'SSL'));
	}
	// Get Module Id new 
	$moduleid_new= $this->model_extension_so_theme_module_so_deals->getModuleId();
	$module_id = '';	
	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
		if (!isset($this->request->get['module_id'])) {
			$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
			$module_id = $moduleid_new[0]['Auto_increment'];
			$this->model_setting_module->addModule('so_theme.so_deals', $this->request->post);
		...
		if($action == "save_edit") {
			$this->response->redirect($this->url->link('extension/so_theme/module/so_deals', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
	...
	if (!isset($this->request->get['module_id'])) {
		$data['action'] = $this->url->link('extension/so_theme/module/so_deals', 'user_token=' . $this->session->data['user_token'], 'SSL');
	} else {
		$data['action'] = $this->url->link('extension/so_theme/module/so_deals', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
	}
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_theme.so_deals' );
		$data['link'] = $this->url->link('extension/so_theme/module/so_deals', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		...
		$data['store_layouts'] = array(
			...
			'default2x' 	=> $this->language->get('value_default2x'),
			'default2x_2' 	=> $this->language->get('value_default2x_2'),
			'default2x_3' 	=> $this->language->get('value_default2x_3'),
			'default2x_4' 	=> $this->language->get('value_default2x_4'),
		...
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_deals', $data));
	}
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_deals', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_deals', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_deals')) {
		...
		if (isset($this->request->post['display_feature']) && $this->request->post['display_feature'] == 1 && $this->request->post['product_feature'] == null) {
		...
		if (!filter_var($this->request->post['count_limit_random'],FILTER_VALIDATE_FLOAT) || $this->request->post['count_limit_random'] < 0 || $this->request->post['count_limit_random'] > 100) {
			$this->error['count_limit_random'] = $this->language->get('error_count_limit_random');
		}			
	...
	public function autocomplete_category() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_so_theme_module_so_deals->getCategories($filter_data);
	...
	public function autocomplete_product_feature() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_so_theme_module_so_deals->getProducts_deals($filter_data);
			foreach ($results as $result) {
				$json[] = array(
