...
class SoHomeSlider extends \Opencart\System\Engine\Controller {	
	...
	public function index() {
		...
		$this->load->language('extension/so_theme/module/so_home_slider','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
		$data['objlang'] = $this->language;	
	// Load model	
		$this->load->model('catalog/category');
		$this->load->model('setting/module');
		$this->load->model('extension/so_theme/module/so_home_slider');
		$this->load->model('tool/image');
		
		$this->document->setTitle($this->language->get('heading_title'));
		
	// Get data
		$model = $this->model_extension_so_theme_module_so_home_slider; 
		$model->checkInstall();
		
	$chk = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "so_homeslider_description` WHERE `Field` = 'image_lang'");
	...
	{
		// Add js
		$this->document->addScript('../extension/so_theme/admin/view/javascript/so_home_slider/js/jquery-ui.min.js');
		$this->document->addStyle('../extension/so_theme/admin/view/javascript/so_home_slider/js/jquery-ui.min.css');
	}
	...
	// Delete Module
		if( isset($this->request->get['slide_id']) && isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			$this->model_extension_so_theme_module_so_home_slider->deleteSlide( $this->request->get['slide_id'] );
			$this->response->redirect($this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'].'&tab=slide&module_id='.$this->request->get['module_id'], 'SSL'));
			...
			$this->model_extension_so_theme_module_so_home_slider->deleteAllSlide($this->request->get['module_id']);
			$this->response->redirect($this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
	// Get Module Id New
		$moduleid_new= $this->model_extension_so_theme_module_so_home_slider->getModuleId(); // Get module id
		$module_id = '';	
		$slideid_new= $this->model_extension_so_theme_module_so_home_slider->getSlideId(); // Get slide id
		$slide_id = '';
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($tab_module == 'add-slide' || $tab == 'slide' && $tab_module != 'add-module'  )
			{
				...
				if (!isset($this->request->get['slide_id'])) {
					...
					$this->model_extension_so_theme_module_so_home_slider->addSlide($this->request->post);
				} else {
					$slide_id = $this->request->get['slide_id'];
					$this->request->post['moduleid'] = $this->request->get['module_id'];
					
					$this->model_extension_so_theme_module_so_home_slider->editSlide($this->request->get['slide_id'], $this->request->post);
				}
				
				if($action == "save_edit") {
					$this->response->redirect($this->url->link('extension/so_theme/module/so_home_slider', 'module_id='.$this->request->get['module_id'].'&tab=slide&slide_id='.$slide_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
				...
				if (!isset($this->request->get['module_id'])) {
					...
					$this->model_setting_module->addModule('so_theme.so_home_slider', $this->request->post);
				...
				if($action == "save_edit") {
					$this->response->redirect($this->url->link('extension/so_theme/module/so_home_slider', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
				}elseif($action == "save_new"){
					$this->response->redirect($this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'], 'SSL'));
				}else{
					$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		...
		$data['no_img'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		...
		if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST') || ($this->request->server['REQUEST_METHOD'] == 'POST' && !$this->validate() && isset($this->request->get['module_id'])) || ($this->request->server['REQUEST_METHOD'] == 'POST' && !$this->validate() && isset($this->request->get['slide_id']))) {
			...
			$data['action'] 				= $this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
			
			if(isset($this->request->get['slide_id'])){
				$data['action'] = $this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '&tab=slide&module_id='.$this->request->get['module_id'].'&slide_id=' . $this->request->get['slide_id'], 'SSL');
				
			}elseif(isset($this->request->get['tab']) && $this->request->get['tab'] == "slide")
			{
				$data['action'] = $this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '&tab=slide&module_id='.$this->request->get['module_id'], 'SSL');
			}
			...
			if(isset($this->request->get['slide_id'])){
				$data['slide_description'] = $this->model_extension_so_theme_module_so_home_slider->getSliderById($this->request->get['slide_id']);

				$language = count($data['slide_description']);
				$language_id = (int)$this->config->get('config_language_id');
									
				
				foreach ($data['slide_description'] as $key => $item) {										
						if ($item['image_lang'] != '') {						
							$data['slide_description'][$key]['thumb_lang'] = $this->model_tool_image->resize($item['image_lang'], 100, 100);
						} else {
							$data['slide_description'][$key]['thumb_lang'] = $this->model_tool_image->resize('no_image.png', 100, 100);
						}
				}
			...
			if($this->request->post != null){				
				...
				foreach($this->request->post['slide_description'] as $item){
					//var_dump($item);die();
						...
						'image_lang'		=> $item['image_lang'],
						'url_lang'		    => $item['url_lang'],
						'status_lang'		=> $item['status_lang'],
			...
			$data['selectedid'] 	= 0;
			$data['action'] 		= $this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_theme.so_home_slider' );
		$data['link'] = $this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/so_theme/module/so_home_slider&user_token=' . $this->session->data['user_token']);
		//--------------------------------Load Data -------------------------------------------
		...
		$data['store_layouts'] = array(
			...
			'home21' 	=> $this->language->get('Home Layout 21')	,
		);
		...
		{
			$data['slides_old'] = $model->getListSliderGroups($module_info['moduleid']);
			foreach($data['slides_old'] as $i) {
				
					if (is_file(DIR_IMAGE . $i['image_lang'])) {
						$i['image_lang'] = $this->model_tool_image->resize($i['image_lang'], 1400, 600);
					} else {
						$i['image_lang'] = $this->model_tool_image->resize('no_image.png', 1000, 600);
					}			
				foreach($data['languages'] as $item) {
					if($i['language_id'] === $item['language_id']) {
						$data['slides'][$item['language_id']][] = $i;
					}			
				}
			}			
		}	
		...
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_home_slider', $data));
	}
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_home_slider', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $this->data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_home_slider')) {
		...
		foreach($languages as $language){
			...
			{					
				...
				if ((strlen($slide_description[$language['language_id']]['slide_title']) < 1)) {
		...
		if (isset($this->request->post['image']) && $this->request->post['tab_module'] == 'add-slide' && (strlen($this->request->post['image']) < 1) || isset($this->request->post['image']) && isset($this->request->get['tab']) && ($this->request->get['tab'] == "slide") && (strlen($this->request->post['image']) < 1) && isset($this->request->post['image']) && $this->request->post['tab_module'] == '') {
	...
	public function sortposition() {
		...
		if (isset($this->request->get['sortarray'])) {
			$result = $this->model_extension_so_theme_module_so_home_slider->updatePositionSlide($this->request->get['sortarray']);
			//var_dump($result);die();
		}
	}
	
	public function addImageLang() {
		$this->load->model('extension/so_theme/module/so_home_slider');
		$this->db->query("ALTER TABLE " . DB_PREFIX . "so_homeslider_description ADD image_lang VARCHAR(255) NOT NULL; ");
	    $this->db->query("ALTER TABLE " . DB_PREFIX . "so_homeslider_description ADD url_lang VARCHAR(255) NOT NULL;");
		$this->db->query("ALTER TABLE " . DB_PREFIX . "so_homeslider_description ADD status_lang tinyint(1)  NOT NULL; ");
		$this->db->query("ALTER TABLE " . DB_PREFIX . "so_homeslider_description ADD jusv VARCHAR(11) NOT NULL; ");
	}	
}