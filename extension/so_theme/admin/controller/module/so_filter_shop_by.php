...
class SoFilterShopBy extends \Opencart\System\Engine\Controller {
	...
	public function index() {
		// Load language
		$this->load->language('extension/so_theme/module/so_filter_shop_by','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
		...
		$this->load->model('extension/so_theme/module/so_filter_shop_by');
		...
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/so_theme/module/so_filter_shop_by', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
		
		// Get module id new 
		$moduleid_new= $this->model_extension_so_theme_module_so_filter_shop_by->getModuleId(); // Get module id
		$module_id = '';
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (!isset($this->request->get['module_id'])) {
				...
				$this->model_setting_module->addModule('so_theme.so_filter_shop_by', $this->request->post);
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/so_theme/module/so_filter_shop_by', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/so_theme/module/so_filter_shop_by', 'user_token=' . $this->session->data['user_token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/so_theme/module/so_filter_shop_by', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		// Get all attribute
		$attributes =  $this->model_extension_so_theme_module_so_filter_shop_by->getAttributes();
		if(!empty($attributes)){
		...
		// Get all options
		$options_arr = $this->model_extension_so_theme_module_so_filter_shop_by->getOptions();
		...
		$manufactures = $this->model_extension_so_theme_module_so_filter_shop_by->getMenufacture();
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_theme.so_filter_shop_by' );
		$data['link'] = $this->url->link('extension/so_theme/module/so_filter_shop_by', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/so_theme/module/so_filter_shop_by&user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_filter_shop_by', $data));
	}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_filter_shop_by')) {
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_filter_shop_by', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_filter_shop_by', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
    ...
    private function setupEvent() {
		...
		$this->model_extension_so_theme_module_soconfig_setting->addEvent('so_filter_shop_by','', "catalog/view/product/category/after", "	extension/so_theme/event/so_filter_shop_by.text_after" , 1 , 1);			
    }	

    private function removeEvent() {
        $this->load->model('setting/event');
        $this->model_setting_event->deleteEventByCode('so_filter_shop_by');
    }	
