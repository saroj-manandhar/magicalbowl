...
class SoListingTabs extends \Opencart\System\Engine\Controller {	
	...
	public function index() {
		// Load language
		$this->load->language('extension/so_theme/module/so_listing_tabs','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
		...
		$this->load->model('extension/so_theme/module/so_listing_tabs');
	...
	// Delete Module
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/so_theme/module/so_listing_tabs', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
	// Get Module Id new 
		$moduleid_new= $this->model_extension_so_theme_module_so_listing_tabs->getModuleId();
		$module_id = '';	
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
				$module_id = $moduleid_new[0]['Auto_increment'];
				$this->model_setting_module->addModule('so_theme.so_listing_tabs', $this->request->post);
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/so_theme/module/so_listing_tabs', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/so_theme/module/so_listing_tabs', 'user_token=' . $this->session->data['user_token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/so_theme/module/so_listing_tabs', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'], 'SSL');

		$moduleid_new= $this->model_extension_so_theme_module_so_listing_tabs->getModuleId(); // Get module id
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_theme.so_listing_tabs' );
		$data['link'] = $this->url->link('extension/so_theme/module/so_listing_tabs', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/so_theme/module/so_listing_tabs&user_token=' . $this->session->data['user_token']);
		//--------------------------------Load Data -------------------------------------------
		...
		$data['store_layouts'] = array(
			...
			'default23' 	=> $this->language->get('value_default23')	,
			'default25' 	=> $this->language->get('value_default25')	,
			'default27' 	=> $this->language->get('value_default27')	,
			'default33' 	=> $this->language->get('value_default33')	,
			'default35' 	=> $this->language->get('value_default35')	,
			'default38' 	=> $this->language->get('value_default38')	,
			'category' 	=> $this->language->get('value_category')	
		);		
		...
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_listing_tabs', $data));
	}
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_listing_tabs', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_listing_tabs', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_listing_tabs')) {
		...
		if (strlen($this->request->post['banner_image']) < 1) {
	...
	public function autocomplete_category() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_so_theme_module_so_listing_tabs->getCategories($filter_data);
