...
class SoMegaMenu extends \Opencart\System\Engine\Controller {
    ...
    public function index(): void {
        ...
        $this->load->language('extension/so_theme/module/so_megamenu','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
        $this->document->setTitle($this->language->get('heading_title'));
        $this->load->model('setting/setting');
		
        $this->load->model('extension/so_theme/module/so_megamenu');
		
        $this->load->model('tool/image');
        $this->load->model('setting/module');
		$this->document->addScript('view/javascript/ckeditor/ckeditor.js');
		$this->document->addScript('view/javascript/ckeditor/adapters/jquery.js');
		$this->document->addStyle('../extension/so_theme/admin/view/javascript/so_megamenu/so_megamenu.css');
		$this->document->addScript('../extension/so_theme/admin/view/javascript/so_megamenu/jquery.nestable.js');
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode('so_theme.so_megamenu');
		$data['link_add'] = $this->url->link('extension/so_theme/module/so_megamenu', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		$this->load->model('catalog/category');
		$results = $this->model_catalog_category->getCategories($dataCate = []);
        ...
        if(isset($_GET['delete'])) {
            if($this->validate()){
                if($this->model_extension_so_theme_module_so_megamenu->deleteMenu(intval($_GET['delete']))) {
                    $this->session->data['success'] = 'This menu has been properly removed from the database.';
                } else {
                    $this->session->data['error_warning'] = $this->model_extension_so_theme_module_so_megamenu->displayError();
                }
            } else {
                $this->session->data['error_warning'] = $this->language->get('error_permission');
            }
            $this->response->redirect(HTTP_SERVER . 'index.php?route=extension/so_theme/module/so_megamenu&user_token=' . $this->session->data['user_token'].'&action=create&module_id='.$this->request->get['module_id']);
        }
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            if(isset($_POST['button-create'])) {
                if($this->validate()) {
                    ...
                    if($error == true) {
						...
						if($this->request->post['link'])
							$this->request->post['link'] = json_encode ($this->request->post['link']);
						if(isset($this->request->post['content']['subcategory']['category']) && ($this->request->post['content']['subcategory']['category']))
							$this->request->post['content']['subcategory']['category'] = json_encode ($this->request->post['content']['subcategory']['category']);	
                        $this->model_extension_so_theme_module_so_megamenu->addMenu($this->request->post);
                ...
                $this->response->redirect(HTTP_SERVER . 'index.php?route=extension/so_theme/module/so_megamenu&user_token=' . $this->session->data['user_token']);
            }
        }		
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            if(isset($_POST['button-back'])) {
				$this->response->redirect(HTTP_SERVER . 'index.php?route=extension/so_theme/module/so_megamenu&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id']);
			}	
            ...
            elseif(isset($_POST['button-edit'])) {
                if($this->validate()) {
                    ...
                    if($error == true) {
						...
						if(isset($this->request->post['content']['subcategory']['category']) && ($this->request->post['content']['subcategory']['category']))
							$this->request->post['content']['subcategory']['category'] = json_encode ($this->request->post['content']['subcategory']['category']);						
                        $this->model_extension_so_theme_module_so_megamenu->saveMenu($this->request->post);
                        $this->session->data['success'] = $this->language->get('text_success');
						$this->response->redirect(HTTP_SERVER . 'index.php?route=extension/so_theme/module/so_megamenu&user_token=' . $this->session->data['user_token'].'&edit='.$this->request->post['id'].'&module_id='.$this->request->get['module_id']);
                    }
                } else {
                    $this->session->data['error_warning'] = $this->language->get('error_permission');
                }
        ...
        $data['nestable_list'] = $this->model_extension_so_theme_module_so_megamenu->generate_nestable_list($data['language_id']);
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if(isset($_POST['button-save'])){
                ...
                if(!isset($this->request->post['name'])) 
					$this->request->post['name'] = 'so_mega_menu';
				...
				$moduleid_new= $this->model_extension_so_theme_module_so_megamenu->getModuleId(); // Get module id
				$module_id = '';
				if (!isset($this->request->get['module_id'])) {
					$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
					$module_id = $moduleid_new[0]['Auto_increment'];
					$this->model_setting_module->addModule('so_theme.so_megamenu', $this->request->post);	
				} else {
				...
				if (isset($this->request->post['import_module']) && $this->request->post['import_module']) {
					...
					$this->model_extension_so_theme_module_so_megamenu->duplicateModule($module_id,$import_module);
				}					
                $this->session->data['success'] = $this->language->get('text_success');
				$this->response->redirect($this->url->link('extension/so_theme/module/so_megamenu', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
			}
        }
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_megamenu', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_megamenu', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}	

		$this->load->model('setting/module');
		$data['modules'] = $this->model_setting_module->getModulesByCode('so_theme.so_megamenu');
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('extension/so_theme/module/so_megamenu', 'user_token=' . $this->session->data['user_token'], 'SSL');
			$data['selectedid'] = 0;       
	   } else {
            $data['action'] = $this->url->link('extension/so_theme/module/so_megamenu', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');			
        ...
        if (isset($_GET['jsonstring'])) {
            if($this->validate()){
                ...
                foreach ($readbleArray as $key => $value) {
                    if (is_array($value)) {
                        $this->model_extension_so_theme_module_so_megamenu->save_rang($value['parentID'], $value['id'], $key);
                    }
                }
        ...
        if(isset($_GET['edit'])) {
            ...
            $dane = $this->model_extension_so_theme_module_so_megamenu->getMenu(intval($_GET['edit']));
			...
			if (isset($dane['content']['image']['link']) && is_file(DIR_IMAGE . $dane['content']['image']['link'])) {
				...
				$dane['content']['image']['image_link'] = $this->model_tool_image->resize($dane['icon'], 100, 100);
			} else {
			...
			if($dane) {
                ...
                $data['link'] 		= (isset($dane['link']) && $dane['link']) ? @json_decode($dane['link']) :  array();
                ...
                if(isset($dane['content']['categories']['categories'])) {
					$data['list_categories'] = $this->model_extension_so_theme_module_so_megamenu->getCategories(@$dane['content']['categories']['categories']);
				}	              
            } else {
        ...
        {
            $new_menu_id = $this->model_extension_so_theme_module_so_megamenu->duplicateMenu($_GET['duplicate']);
            $this->response->redirect(HTTP_SERVER . 'index.php?route=extension/so_theme/module/so_megamenu&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id'].'&edit='. $new_menu_id);
        }
        elseif (isset($_GET['changestatus']))
        {
            $dane = $this->model_extension_so_theme_module_so_megamenu->getMenu(intval($_GET['changestatus']));
            if ($dane['status']==1)
                $status['status'] =0;
            else
                $status['status'] =1;
            $status['id'] = intval($_GET['changestatus']);
            $this->model_extension_so_theme_module_so_megamenu->UpdatePosition($status);
            $this->response->redirect(HTTP_SERVER . 'index.php?route=extension/so_theme/module/so_megamenu&user_token=' . $this->session->data['user_token'].'&action=create&module_id='.$this->request->get['module_id']);
        }
        $this->load->model('tool/image');
        ...
        $data['linkremove'] = $this->url->link('extension/so_theme/module/so_megamenu&user_token=' . $this->session->data['user_token']);
		...
		if(isset($this->request->get['module_id'])){
		    $data['linkexport'] =  $this->url->link('extension/so_theme/module/so_megamenu|export_data&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id']);

			$data['linkimport'] =  $this->url->link('extension/so_theme/module/so_megamenu|linkimport&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id']);			
		}
        // Remove cache
        ...
        $this->response->setOutput($this->load->view('extension/so_theme/module/so_megamenu', $data));			
    }
    ...
    public function uninstall() {
        $this->load->model('setting/setting');
        $this->load->model('extension/so_theme/module/so_megamenu');
        $this->model_extension_so_theme_module_so_megamenu->uninstall();
    }
    ...
    public function install() {
        ...
        $this->model_setting_module->addModule('so_theme.so_megamenu',$data);
		$new_module = $this->getModulesByCode('so_theme.so_megamenu');
		if(is_array($new_module))		
			$this->model_extension_so_theme_module_so_megamenu->install($new_module['module_id']);
    }
    ...
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_megamenu')) {
    ...
    public function export_data() {
		...
		$this->load->model('extension/so_theme/module/so_megamenu');
		$query = $this->model_extension_so_theme_module_so_megamenu->getAllMenu($this->request->get['module_id']);
	...
	public function linkimport() {
		...
		if(isset($r) || $r != null || isset($r[0]->name))
		{			
	        
			foreach ($query as $key => $result) {
				$this->model_extension_so_theme_module_so_megamenu->deleteAllMenu($result['id']);
			}
			
			$querys = $this->model_extension_so_theme_module_so_megamenu->getAllMenu($this->request->get['module_id']);

			if($querys == array()) {
				foreach ($r as $key => $data) {
					$f=(array)$data;
					$f['display_submenu'] = $f['submenu_type'];
					$this->model_extension_so_theme_module_so_megamenu->addMenu($f);
				}				
			}
