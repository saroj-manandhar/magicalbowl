...
class SoLatestBlog extends \Opencart\System\Engine\Controller {	
	...
	public function index() {
		// Load language
		$this->load->language('extension/so_theme/module/so_latest_blog','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
		...
		$this->load->model('extension/so_theme/module/so_latest_blog');
		$this->load->model('cms/topic');
			...
			if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
				...
				$this->response->redirect($this->url->link('extension/so_theme/module/so_latest_blog', 'user_token=' . $this->session->data['user_token'], 'SSL'));
			}
			// Get Module Id new 
			$moduleid_new= $this->model_extension_so_theme_module_so_latest_blog->getModuleId(); 
			$module_id = '';
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
				if (!isset($this->request->get['module_id'])) {
					$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
					$module_id = $moduleid_new[0]['Auto_increment'];
					$this->model_setting_module->addModule('so_theme.so_latest_blog', $this->request->post);
				...
				if($action == "save_edit") {
					$this->response->redirect($this->url->link('extension/so_theme/module/so_latest_blog', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
			...
			if (!isset($this->request->get['module_id'])) {
				$data['action'] = $this->url->link('extension/so_theme/module/so_latest_blog', 'user_token=' . $this->session->data['user_token'], 'SSL');
			} else {
				$data['action'] = $this->url->link('extension/so_theme/module/so_latest_blog', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
			}
			...
			if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST') || $this->request->server['REQUEST_METHOD'] == 'POST' && !$this->validate() && isset($this->request->get['module_id'])) {
				...
				foreach ($categorys as $category_id) {
					$category_info_1 = $this->model_cms_topic->getTopic($category_id);				
					if ($category_info_1) {
						$module_info['categorys'][] = array(
							'simple_blog_category_id' 	=> $category_info_1['topic_id'],
							'name'       				=> $category_info_1['name']
						); 
					}
				}
				$data['action'] = $this->url->link('extension/so_theme/module/so_latest_blog', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
				...
				if($this->request->post != null)
				{
					...
					{
						foreach ($categorys as $category_id) {
							$category_info_1 = $this->model_extension_so_theme_module_simple_blog_category->getCategory($category_id);
			...
			$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_theme.so_latest_blog' );
			$data['link'] = $this->url->link('extension/so_theme/module/so_latest_blog', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
			$data['linkremove'] = $this->url->link('extension/so_theme/module/so_latest_blog&user_token=' . $this->session->data['user_token']);
			...
			$this->response->setOutput($this->load->view('extension/so_theme/module/so_latest_blog/so_latest_blog', $data));
	}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_latest_blog')) {
		...
		if ((strlen($this->request->post['readmore_text']) < 1) ) {
	...
	public function getLayoutMod($name=null){
		...
		$log_directory  = DIR_EXTENSION.'so_theme/catalog/view/template/module/'.$name;
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_latest_blog', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_latest_blog', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($results as $result) {
				$json[] = array(
					'simple_blog_category_id' 	=> $result['topic_id'],
