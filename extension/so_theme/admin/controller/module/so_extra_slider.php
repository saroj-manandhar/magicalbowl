...
class SoExtraSlider extends \Opencart\System\Engine\Controller {
	...
	// Load language	
		$this->load->language('extension/so_theme/module/so_extra_slider','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
	...
	// Load Model
		...
		$this->load->model('extension/so_theme/module/so_extra_slider');
	...
	// Delete Module
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/so_theme/module/so_extra_slider', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}	
	// Get Module Id New
		$moduleid_new= $this->model_extension_so_theme_module_so_extra_slider->getModuleId();
		$module_id = '';	
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
				$module_id = $moduleid_new[0]['Auto_increment'];
				$this->model_setting_module->addModule('so_theme.so_extra_slider', $this->request->post);
			} else {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/so_theme/module/so_extra_slider', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/so_theme/module/so_extra_slider', 'user_token=' . $this->session->data['user_token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/so_theme/module/so_extra_slider', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_theme.so_extra_slider' );
		$data['link'] = $this->url->link('extension/so_theme/module/so_extra_slider', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/so_theme/module/so_extra_slider&user_token=' . $this->session->data['user_token']);
		...
		$data['store_layouts'] = array(
			...
			'default3' 	=> $this->language->get('value_default3')	,
			'default4' 	=> $this->language->get('value_default4')	,
			'default5' 	=> $this->language->get('value_default5')	,
			'default6' 	=> $this->language->get('value_default6')	,
			'default7' 	=> $this->language->get('value_default7')	,
			'default15' 	=> $this->language->get('value_default15')	,			
			'default17' 	=> $this->language->get('value_default17')	,
			'default24' 	=> $this->language->get('value_default24')	,
			'default27' 	=> $this->language->get('value_default27')	,			
			'default28' 	=> $this->language->get('value_default28')	,
			'default31' 	=> $this->language->get('value_default31')	,
			'default32' 	=> $this->language->get('value_default32')	,
			'default34' 	=> $this->language->get('value_default34')	,
			'default35' 	=> $this->language->get('value_default35')	,
			'default36' 	=> $this->language->get('value_default36')	,
			'simple' 	=> $this->language->get('value_simple')	
		...
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_extra_slider', $data));
	}
	...
	public function _breadcrumbs()
	{
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_extra_slider', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_extra_slider', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_extra_slider')) {
		...
		if (strlen($this->request->post['placeholder_path']) < 1) {
	...
	public function autocomplete_category(): void {
		$json = array();
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_extension_so_theme_module_so_extra_slider->getCategories($filter_data);
