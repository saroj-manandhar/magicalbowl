...
class SoSuperCategory extends \Opencart\System\Engine\Controller {	
	...
	// Load language
		$this->load->language('extension/so_theme/module/so_super_category','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
	...
	// Delete Module
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/so_theme/module/so_super_category', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
	//Get Module Id New
		$moduleid_new= $this->model_extension_so_theme_module_so_super_category->getModuleId();
		$module_id = '';	
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (!isset($this->request->get['module_id'])) {
				...
				$this->model_setting_module->addModule('so_theme.so_super_category', $this->request->post);
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/so_theme/module/so_super_category', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/so_theme/module/so_super_category', 'user_token=' . $this->session->data['user_token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/so_theme/module/so_super_category', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$categorys_list = $this->model_extension_so_theme_module_so_super_category->getCategories($filter_data);
	...
	// ---------------------------Load module --------------------------------------------
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_theme.so_super_category' );
		$data['link'] = $this->url->link('extension/so_theme/module/so_super_category', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/so_theme/module/so_super_category&user_token=' . $this->session->data['user_token']);
	// ---------------------------Load data ----------------------------------------------
	...
	//Get Data Default
		...
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_super_category', $data));
	}
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_super_category', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_super_category', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $this->data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_super_category')) {
	...
	public function autocomplete() {
		$json = array();
		$this->load->language('extension/so_theme/module/so_super_category');
