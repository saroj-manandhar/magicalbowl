...
require_once (DIR_EXTENSION.'so_theme/admin/view/template/module/so_page_builder'.DIRECTORY_SEPARATOR.'data.php');
require_once (DIR_EXTENSION.'so_theme/admin/view/template/module/so_page_builder'.DIRECTORY_SEPARATOR.'addElement.php');
require_once (DIR_EXTENSION.'so_theme/admin/view/template/module/so_page_builder'.DIRECTORY_SEPARATOR.'formField.php');
...
class SoPageBuilder extends \Opencart\System\Engine\Controller {
	...
	public function index() {
		...
		if(isset($this->request->get['module_id'])){
		    $data['linkexport'] =  $this->url->link('extension/so_theme/module/so_page_builder|export_data&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id']);

			$data['linkimport'] =  $this->url->link('extension/so_theme/module/so_page_builder|linkimport&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id']);			
		}

		/* Load language */ 
		$this->load->language('extension/so_theme/module/so_page_builder','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
		...
		$data['this_module_id'] = $module_id;

		/* Load Template */ 
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_page_builder', $data));
	}
	...
	public function _dataDefaultModule($module_id){	
		...
		$data['moduletabs'] 	= $this->model_setting_module->getModulesByCode( 'so_theme.so_page_builder' );
		$data['link'] 			= $this->url->link('extension/so_theme/module/so_page_builder', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
	...
	public function _getDataModule($module_id){
		...
		if ($module_id != "") {
			...
			$data['action'] = $this->url->link('extension/so_theme/module/so_page_builder', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $module_id, 'SSL');			
			...
			$data['selectedid'] = 0;
			$data['action'] = $this->url->link('extension/so_theme/module/so_page_builder', 'user_token=' . $this->session->data['user_token'], 'SSL');
	...
	public function _saveAllModule($module_id,$action){
		...
		if ($module_id == "") {
			$moduleid_new= $this->model_extension_so_theme_module_so_page_builder->getModuleId(); // Get module id
			$module_id = $moduleid_new[0]['Auto_increment'];
			/* Write File Css */
			$this->_writeFileCss($module_id,$page_builder);
			$this->model_setting_module->addModule('so_theme.so_page_builder', $this->request->post);		
		} else {
			/* Write File Css */
		...
		switch($action){
			...
			break;
			case "save_edit":
				$this->response->redirect($this->url->link('extension/so_theme/module/so_page_builder', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
			break;
			case "save_new":
				$this->response->redirect($this->url->link('extension/so_theme/module/so_page_builder', 'user_token=' . $this->session->data['user_token'], 'SSL'));
			break;
		}
		return $module_id;
	}
	...
	public function _deleteModule($module_id){
		if($this->_checkPermission()){
			...
			$file = DIR_EXTENSION.'so_theme/catalog/view/javascript/so_page_builder/css/style_render_'.$module_id.'.css';
			unlink($file);
	...
	public function _duplicateModule($module_id){
		if($this->_checkPermission()){
			$module_id_arr = $this->model_extension_so_theme_module_so_page_builder->getModuleId(); // Get module id
			$module_id_dup = $module_id_arr[0]['Auto_increment'];
			$this->model_extension_so_theme_module_so_page_builder->duplicateModule($module_id);
			/*Duplicate file css for Module Id*/
			$file = DIR_EXTENSION.'so_theme/catalog/view/javascript/so_page_builder/css/style_render_'.$module_id.'.css';
			$new_file = DIR_EXTENSION.'so_theme/catalog/view/javascript/so_page_builder/css/style_render_'.$module_id_dup.'.css';
			copy($file,$new_file);
			$this->session->data['success'] = $this->language->get('text_success_duplicate');
			$this->response->redirect($this->url->link('extension/so_theme/module/so_page_builder', 'module_id='.$module_id_dup.'&user_token=' . $this->session->data['user_token'], 'SSL'));				
		}else{
			$this->session->data['warning'] = $this->language->get('error_permission');
		}	
		return $module_id;
	...
	public function _importData($import_theme){
		if($this->_checkPermission()){
			...
			$setting = $this->model_extension_so_theme_module_so_page_builder->importSimpleData($import_theme,$dataExtensions); //Import theme
	...
	public function _getModuleInstall(){
		...
		$extensions = $this->model_extension_so_theme_module_so_page_builder->getInstalled('module');

		// Add all the modules which have multiple settings for each module
			
		foreach ($extensions as $code) {
			if( $code['code']=="so_page_builder"){
				continue;
			}
		
	    	$this->load->language('extension/'.$code['extension'].'/module/' . $code['code'],'',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
			$module_data = array();
			
			$modules = $this->model_setting_module->getModulesByCode($code['extension'].'.'.$code['code']);
			...
			foreach ($modules as $module) {
				$module_data[] = array(
					...
					'module'=> $code['extension'] . '.' . $code['code'] . '.' .  $module['module_id'],
					'id' 	=>  $module['module_id'],
			...
			if( $modules  ){
				if ($this->config->has($code['code'] . '_status') || $module_data) {					
					$data['extensions'][$code['code']] = array(
	...
	public function _loadStyleAndScript(){
		$this->document->addStyle('../extension/so_theme/admin/view/javascript/so_page_builder/js/ui/jquery-ui.min.css');
		$this->document->addStyle('../extension/so_theme/admin/view/javascript/so_page_builder/css/farbtastic.css');
		$this->document->addStyle('../extension/so_theme/admin/view/javascript/so_page_builder/css/simpleslider.css');
		$data['direction'] = $this->language->get('direction');
		if ($data['direction'] != 'rtl') $this->document->addStyle('../extension/so_theme/admin/view/javascript/so_page_builder/css/style.css');
		else $this->document->addStyle('extension/so_theme/admin/view/javascript/so_page_builder/css/style-rtl.css');
		$this->document->addScript('../extension/so_theme/admin/view/javascript/so_page_builder/js/ui/jquery-ui.min.js');
		$this->document->addScript('../extension/so_theme/admin/view/javascript/so_page_builder/js/farbtastic.js');
		$this->document->addScript('../extension/so_theme/admin/view/javascript/so_page_builder/js/simpleslider.js');
		$this->document->addScript('../extension/so_theme/admin/view/javascript/so_page_builder/js/so_page_builder.js');
		$this->document->addStyle('../extension/so_theme/admin/view/javascript/so_page_builder/css/select2.min.css');
		$this->document->addScript('../extension/so_theme/admin/view/javascript/so_page_builder/js/select2.min.js');
		
	}
	
	/* Check Validate Module*/
	protected function _validate(){
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_page_builder')) {
	...
	public function _breadcrumbs($module_id){
		...
		if ($module_id == "") {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_page_builder', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_page_builder', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $module_id, 'SSL')
			);
		}
	...
	// public function _previewPage(){
		...
		// $return = array();
		// //$return['html'] = $this->load->controller('catalog/extension/so_theme/module/so_page_builder', $data_page);
...
/*------Shortcode------------------Shortcode---------------------Shortcode---------------*/
	...
	public function _getFormShortcode(){
		...
		$database['category'] = $this->model_catalog_category->getCategories(array());
	...
	protected function _callShortcode($shortcode,$module_id){
		...
		$content =  json_decode(html_entity_decode($shortcode->content, ENT_COMPAT, 'UTF-8'));
		...
		switch ($func_shortcode) {
			case "countdownYTShortcode":
			    countdownYTShortcode($contentP[0],$contentC,$module_id,$shortcode->module,$database);
			break;
			case "accordionYTShortcode":
			    accordionYTShortcode($contentP[0],$contentC,$module_id,$shortcode->module,$database);
			break;	
			case "accordion_itemYTShortcode":
			    accordion_itemYTShortcode($contentP[0],$contentC,$module_id,$shortcode->module,$database);
			break;				
		}		
    ...
    public function export_data() {
		$this->load->model('extension/so_theme/module/so_page_builder');
		if($this->_checkPermission()){
			$query = $this->model_extension_so_theme_module_so_page_builder->exportModule($this->request->get['module_id']);
			$query->row['module_id'] = '';			
			$name = 'page_builder.json';			
	...
	public function linkimport() {		
		...
		$module_id_dup = $module_id_arr[0]['Auto_increment'];
		$data = array();
		if(isset($r) || $r != null || isset($r->row->name))
		{
			$this->model_extension_so_theme_module_so_page_builder->updateModule($this->request->get['module_id']);
            $this->model_extension_so_theme_module_so_page_builder->importModule($this->request->get['module_id'], (array)$r->row);
			
			$this->response->redirect($this->url->link('extension/so_theme/module/so_page_builder', 'module_id='.$this->request->get['module_id'].'&user_token=' . $this->session->data['user_token'], 'SSL'));
        }
		else {
