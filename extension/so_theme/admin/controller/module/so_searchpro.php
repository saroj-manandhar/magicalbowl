...
class SoSearchpro extends \Opencart\System\Engine\Controller {
	...
	// Load language
		$this->load->language('extension/so_theme/module/so_searchpro','',isset($this->request->cookie['language'])?$this->request->cookie['language']:$this->config->get('config_language'));
	...
	// Load breadcrumbs
		...
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/so_theme/module/so_searchpro', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
		
		$moduleid_new= $this->model_extension_so_theme_module_so_searchpro->getModuleId(); // Get module id
		$module_id = '';
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->request->post['moduleid'] = $moduleid_new[0]['Auto_increment'];
				$module_id = $moduleid_new[0]['Auto_increment'];
				$this->model_setting_module->addModule('so_theme.so_searchpro', $this->request->post);
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/so_theme/module/so_searchpro', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/so_theme/module/so_searchpro', 'user_token=' . $this->session->data['user_token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/so_theme/module/so_searchpro', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$data['moduletabs'] = $this->model_setting_module->getModulesByCode( 'so_theme.so_searchpro');
		$data['link'] = $this->url->link('extension/so_theme/module/so_searchpro', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
		$data['linkremove'] = $this->url->link('extension/so_theme/module/so_searchpro&user_token=' . $this->session->data['user_token']);
		
		// Keyword
		if (isset($this->request->post['show_keyword'])) {
			$data['show_keyword'] = $this->request->post['show_keyword'];
		} elseif (!empty($module_info)) {
			$data['show_keyword'] = (isset($module_info['show_keyword']) ? $module_info['show_keyword'] : 0);
		} else {
			$data['show_keyword'] = 1;
		}		
		...
		$this->response->setOutput($this->load->view('extension/so_theme/module/so_searchpro', $data));
	}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/so_theme/module/so_searchpro')) {
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/so_theme/module/so_searchpro', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/so_theme/module/so_searchpro', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
