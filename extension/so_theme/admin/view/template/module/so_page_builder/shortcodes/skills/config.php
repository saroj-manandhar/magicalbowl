...
class YT_Shortcode_skills_config {
	static function get_config($language,$value) {
		...
		$name_shortcode = (is_array($value) && isset($value[0]['name_shortcode']) ? $value[0]['name_shortcode'] : $language->get('shortcode_our_skills'));
		...
		return array(
			'name_shortcode' => array(
				'type' 		=> 'textLanguage',
				'default' 	=> (is_array($value) && isset($value[0]['name_shortcode']) ? $value[0]['name_shortcode'] : $language->get('shortcode_our_skills')),
			...
			'width' => array(
				'default' 	=> (is_array($value) && isset($value[0]['width']) ? $value[0]['width'] : ''),
			...
			'no_number' => array(
				'type' 		=> 'bool',
				'default' 	=> (is_array($value) && isset($value[0]['no_number']) ? $value[0]['no_number'] : 'yes'),
				'name' 		=> $language->get('shortcode_skill_no_number'),
				'desc' 		=> $language->get('shortcode_skill_no_number_desc')
			),
			'yt_class'=> array(
