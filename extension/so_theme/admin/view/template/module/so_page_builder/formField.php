...
public static function formField($id,$database,$field=array())
	{
		$html = '';
...
/*  /////////////////////////////// #media /////////////////////////////////// */
			case 'media':
				...
				$html .= '<a  style="display: inline-block;text-align: center;" data-oc-target="#uploadimage'.$database['dem'].'" data-oc-thumb="#thumb-image'.$database['dem'].'"  data-oc-toggle="image" class="img-thumbnail thumb-image"><img class="img-class" data-oc-placeholder="../image/cache/no_image-100x100.png" id="thumb-image'.$database['dem'].'" src="'.$database['url'].'../image/'.resize($field['default'], 100, 100).'" alt="" title="" data-placeholder="'.$database['url'].'image/'.resize('no_image.png', 100, 100).'" width="100" height="100" />		
				</a><input class="form-control imageuploaded yt-generator-attr uploadimage" type="hidden"  name="'.$id.'" id="uploadimage'.$database['dem'].'" value="'.$field['default'].'"/>	
                <div class="card-body">
					<button type="button" data-oc-toggle="image" data-oc-target="#uploadimage'.$database['dem'].'" data-oc-thumb="#thumb-image'.$database['dem'].'" class="btn btn-primary btn-sm btn-block"><i class="fas fa-pencil-alt"></i> Edit</button>
					<button type="button" data-oc-toggle="clear" data-oc-target="#uploadimage'.$database['dem'].'" data-oc-thumb="#thumb-image'.$database['dem'].'" class="btn btn-warning btn-sm btn-block"><i class="fas fa-trash-alt"></i> Clear</button>
                </div>				
				';
			break;	
/*  /////////////////////////////// #text /////////////////////////////////// */
...
/*  /////////////////////////////// #text /////////////////////////////////// */
			case 'textLanguage':
				...
				if($liFirst == 1){
					$html .= '<li>';
				    $html .= '<button class="nav-link active" data-bs-toggle="tab" data-bs-target=".language'.$language['language_id'].'_'.$id.'" type="button" role="tab" aria-controls="nav-module" aria-selected="true">';
							if(version_compare(VERSION, '2.1.0.2', '>')) { 
								$html .= '<img src="language/'.$language['code'].'/'.$language['code'].'.png" title="'.$language['name'].'" />';
							} else { 
								$html .= '<img src="view/image/flags/'.$language['image'].'" title="'.$language['name'].'" />';
							}
							$html .= ' '.$language['name'];
					$html .= '</button>';					
				}else{
					$html .= '<li>';
				    $html .= '<button class="nav-link" data-bs-toggle="tab" data-bs-target=".language'.$language['language_id'].'_'.$id.'" type="button" role="tab" aria-controls="nav-module" aria-selected="true">';
							if(version_compare(VERSION, '2.1.0.2', '>')) { 
								$html .= '<img src="language/'.$language['code'].'/'.$language['code'].'.png" title="'.$language['name'].'" />';
							} else { 
								$html .= '<img src="view/image/flags/'.$language['image'].'" title="'.$language['name'].'" />';
				...
				foreach ($database['languages'] as $language) {
					...
					if($tabContent == 1){
						$html .= '<div class="tab-pane fade show language'.$language['language_id'].'_'.$id.' active" id="content_html_{{ language.language_id }}" role="tabpanel" aria-labelledby="module-tab">';
					}else{
						$html .= '<div class="tab-pane fade show language'.$language['language_id'].'_'.$id.'" id="content_html_{{ language.language_id }}" role="tabpanel" aria-labelledby="module-tab">';
					}
					...
					$html .= '<div class="yt-generator-isp-source yt-generator-isp-source-media '.$classOpenMedia.'">';
						...
						if($slides != '')
						{
							...
							foreach($slides as $slide)
							{
								...
								$html .= '<span><a class="img-thumbnail" data-oc-toggle="image" ><img width="100" height="100" class="img-class" data-oc-placeholder="../image/cache/no_image-100x100.png" title="" alt="" src="'.HTTP_CATALOG.'image/'.$image.'" id="thumb-image'.$key.'" ></a><input type="hidden" id="uploadimage'.$key.'" name="media_image{}" data-base="'.HTTP_CATALOG.'image/" value="'.$slide['image'].'" class="form-control"><i class="fa fa-times"></i></span>
								<div class="card-body">
									<button type="button" data-oc-toggle="image" data-oc-target="#uploadimage'.$key.'" data-oc-thumb="#thumb-image'.$key.'" class="btn btn-primary btn-sm btn-block"><i class="fas fa-pencil-alt"></i> Edit</button>
									<button type="button" data-oc-toggle="clear" data-oc-target="#uploadimage'.$key.'" data-oc-thumb="#thumb-image'.$key.'" class="btn btn-warning btn-sm btn-block"><i class="fas fa-trash-alt"></i> Clear</button>
								</div>';									
							}
						}
						$html .= '</div>';
					...
					$html .= '<div class="yt-generator-isp-source yt-generator-isp-source-media yt-generator-isp-source-open">';
						...
						if($slides != '')
						{
							...
							foreach($slides as $slide)
							{
								...
								$image = resize($slide['image'], 100, 100);
								$placeholder = resize('no_image.png', 100, 100);
								$html .= '<div style="display:flex"><span><a class="img-thumbnail" data-oc-toggle="image" ><img width="100" height="100" class="img-class" data-oc-placeholder="'.HTTP_CATALOG.'image/'.$placeholder.'" title="" alt="" src="'.HTTP_CATALOG.'image/'.$image.'" id="thumb-image'.$key.'" ></a><input type="hidden" id="uploadimage'.$key.'" name="media_image{}" data-base="'.HTTP_CATALOG.'image/" value="'.$slide['image'].'" class="form-control"><i class="fa fa-times"></i></span>
