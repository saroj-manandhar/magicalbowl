...
class So_Fields {
	...
	*/
	...
	public function field_radio($key,$name,$options,$valueDefault=null,$className=null) {
		...
		$radios[] = '<div class="btn-group btn-toggle border rounded '.$className.'" data-ds-toggle="buttons">';
		foreach ($options as $value => $description) {
			if($value ==  $selected){
				$radios[] = '<label class="btn btn-default btn-sm btn-success active"><input class="btn-check field-'.$name.'" type="radio" name="'.$names.'" value="'.$value.'" checked="checked" /> ' . $description . '</label>';
			}else{
				$radios[] = '<label class="btn btn-default btn-sm "><input class="btn-check field-'.$name.'" type="radio" name="'.$names.'" value="'.$value.'" /> ' . $description . '</label>';
			}
		}
		$radios[]=	'</div>';
	...
	*/
	function field_onOffFont($key,$name) {
		...
		$fieldOnOff  = '<div class="btn-group btn-toggle block-group border rounded" data-toggle="buttons">';
		$fieldOnOff .= '<label class="btn btn-default btn-sm '.$onClassActive.'">';
		$fieldOnOff .= '<input class="type-fonts btn-check" type="radio" name="'.$names.'" value="standard" '.$onCheck.'>Standard';
		$fieldOnOff .= '</label>';
		$fieldOnOff .= '<label class="btn btn-default btn-sm '.$offClassActive.'">';
		$fieldOnOff .= '<input class="type-fonts btn-check" type="radio" name="'.$names.'" value="google" '.$offCheck.'>Google Fonts';
		$fieldOnOff .= '</label>';
		$fieldOnOff .=	'</div>';
		return $fieldOnOff;
	}
	...
	function field_typelayout($nameconfig,$element,$element_array,$validate,$columns= null){
		$config = $this-> config;
		$names	= $nameconfig.'['.$element.']';
		$values	= isset($config[$nameconfig][$element]) ? $config[$nameconfig][$element] : '';
		...
		foreach ($element_array as  $fc){
			...
			if(empty ($validate)) $fieldType  .= '<p class="applyLayout btn btn-success" data-keylayout="'.$keylayout.'">Apply Default Setting</p>';
	...
	*/
	...
	function field_date($nameconfig,$element,$placeholder=null){
		...
		$fielddate .='<input type="text" name="'.$names.'" value="'.$values.'" placeholder="'.$placeholder.'" data-date-format="YYYY-MM-DD"  id="input-date-comingsoon" class="form-control date" />';
		$fielddate .='<span class="input-group-btn"><div class="input-group-text"><i class="fa fa-calendar"></i></div> </span>';
		$fielddate .='</div>';
		return $fielddate;
	...
	function field_addimage($nameconfig,$element,$className=null){
		...
		$fieldimage ='<a data-oc-target="#input-'.$element.'" data-oc-thumb="#thumb-'.$element.'" data-oc-toggle="image" class="img-thumbnail card image thumb-image '.$className.'">';
		$fieldimage .='<img class="img-class" src="'.$srcimage.'"  data-oc-placeholder="Background Image:" id="thumb-'.$element.'" />';
		
		$fieldimage .='</a>';
		$fieldimage .='<input type="hidden" name="'.$names.'" value="'.$values.'" id="input-'.$element.'" />';		
		$fieldimage .='
				<div class="card-body">
				<button type="button" data-oc-toggle="image" data-oc-target="#input-'.$element.'" data-oc-thumb="#thumb-'.$element.'" class="btn btn-primary btn-sm btn-block"><i class="fas fa-pencil-alt"></i> Edit</button>
				<button type="button" data-oc-toggle="clear" data-oc-target="#input-'.$element.'" data-oc-thumb="#thumb-'.$element.'" class="btn btn-warning btn-sm btn-block"><i class="fas fa-trash-alt"></i> Clear</button>
				</div>';		
	...
	public function field_devices($nameconfig,$element,$element_array,$column_array=null){
		...
		$fielddevices ='<ul class="nav nav-tabs" role="tablist">';
			...
			$fielddevices .='<li><a class="nav-link '.$navActive.'" data-bs-target="#language-'.$element.$subfix.'" role="tab" data-bs-toggle="tab">'.$device.'</a></li>';
			endforeach;
		$fielddevices .='</ul>';
		...
		$fielddevices .='<div class="tab-content">';
			...
			$fielddevices .='<select name="'.$names.'"  class="form-select width30">';
	...
	function field_select($nameconfig,$element,$element_array,$className=null){
		...
		$fieldSelect  = '<select name="'.$names.'"  class="form-select '.$className.'">';
		foreach ($element_array as $fv => $fc){
	...
	function field_langHori($languages,$nameconfig,$element,$size=null,$className=null){
		...
		$fieldLangHori  .= '<ul class="nav nav-tabs main_tabs_horizontal border rounded">';
		
		foreach ($languages as $language){
			$active = $language['language_id'] == 1 ? 'active' : ' ';
			$fieldLangHori .= '<li class="nav-item">
			<a class="nav-link '.$active.'" data-bs-target="#language-'.$element.$language['language_id'].'" data-bs-toggle="tab"><img src="language/'.$language['code'].'/'.$language['code'].'.png" title="'.$language['name'].'" /> '.$language['name'].'</a>
			</li>';
		}
		$fieldLangHori .= '</ul>';
	...
	function field_langAddMenu($languages,$nameconfig,$element,$valuemenu,$menu_row,$name=null){
		...
		foreach ($languages as $language){
			$active = $language['language_id'] == 1 ? 'active' : ' ';
			$fieldLangHori .= '<li>
			<a class="nav-link '.$active.'" data-bs-target="#language-'.$name.$menu_row.$language['language_id'].'" data-bs-toggle="tab"><img src="language/'.$language['code'].'/'.$language['code'].'.png" title="'.$language['name'].'" /> '.$language['name'].'</a>
			</li>';
		}
		$fieldLangHori .= '</ul>';
		$fieldLangHori .= '<div class="tab-content">';
	...
	function field_langTextarea($languages ,$nameconfig,$element){
		...
		foreach ($languages as $language){
			...
			$fieldLangTextarea .='<textarea name="'.$names.'" data-oc-toggle="ckeditor" data-lang="" id="input-description-'.$element.$language['language_id'].'">';
	...
	public function create_position($titlepositon=null,$namepositon=null,$layout_modules=null,$extensions=null,$module_row =null) {
		...
		foreach ($layout_modules as $layout_module) { 
			if ($layout_module['position'] == $namepositon) {
				...
				$create_position .= '<tr id="module-row'.$module_row.'"><td class="text-left"><div class="input-group input-group-sm">';
				$create_position .= '<select name="layout_module['.$module_row.'][code]" class="form-select input-sm select2-input">';
				...
				$create_position .= '<div class="input-group-btn">';
					$create_position .= '<input type="text" size="5" class="form-order text-center input-sm" name="layout_module['. $module_row.'][sort_order]" value="'. $layout_module['sort_order'].'" />';
					$create_position .= '<a href="'. $layout_module['edit'] .'" type="button" data-toggle="tooltip" title="Edit" target="_blank" class="btn btn-primary btn-sm"><i class="fa fa-pencil-alt"></i></a>';
		...
		$create_position .= '<tfoot><tr><td class="text-left"><div class="input-group input-group-sm">';
		$create_position .= ' <select class="form-select input-sm select2-input">';
	...
	public function home_position($titlepositon=null,$namepositon=null,$layout_modules=null,$extensions=null,$module_row =null,$homeLayouts =null) {
		...
		foreach ($layout_modules as $layout_module) { 
			if ($layout_module['position'] == $namepositon) {
				...
				$create_position .= '<input type="hidden" name="layout_module['.$module_row.'][position]" value="'. $layout_module['position'].'" />';
				$create_position .= '<div class="input-group-btn text-end col-md-2"><p style="margin: 0 15px;">Home Layout</p></div>';
				$create_position .= '<div class="input-group-btn col-md-2" >';
					$create_position .= ' <select  name="layout_module['. $module_row.'][sort_order]"  class="form-select select2-input input-sm" style="width: 75%;">';
