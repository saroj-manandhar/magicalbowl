...
class BankTransfer extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		foreach ($languages as $language) {
			$bank = $this->config->get('payment_bank_transfer_bank_' . $language['language_id']);
			if (empty($bank)) {
				$bank = $this->config->get('payment_bank_transfer_bank' . $language['language_id']);
			}
			$data['payment_bank_transfer_bank'][$language['language_id']] = $bank;
		}
	...
	public function save(): void {
		...
		if (!$json) {
			...
			foreach ($languages as $language) {
				if (isset($this->request->post['payment_bank_transfer_bank_' . $language['language_id']])) {
					$this->request->post['payment_bank_transfer_bank' . $language['language_id']] = $this->request->post['payment_bank_transfer_bank_' . $language['language_id']];
				}
			}
