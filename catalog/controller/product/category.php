...
class Category extends \Opencart\System\Engine\Controller {
	...
	public function index(): ?\Opencart\System\Engine\Action {
		...
		if (isset($this->request->get['note'])) {
			$note = (string)$this->request->get['note'];
		} else {
			$note = '';
		}
		...
		if (isset($this->request->get['minPrice'])) {
			$minPrice = (string)$this->request->get['minPrice'];
		} else {
			$minPrice = '';
		}

		if (isset($this->request->get['maxPrice'])) {
			$maxPrice = (string)$this->request->get['maxPrice'];
		} else {
			$maxPrice = '';
		}
		...
		$custom_url = '';
		if ($type !== '') {
			$custom_url .= '&type=' . urlencode($type);
		}
		if ($note !== '') {
			$custom_url .= '&note=' . urlencode($note);
		}
		if ($size !== '') {
			$custom_url .= '&size=' . urlencode($size);
		}
		if ($minPrice !== '') {
			$custom_url .= '&minPrice=' . urlencode($minPrice);
		}
		if ($maxPrice !== '') {
			$custom_url .= '&maxPrice=' . urlencode($maxPrice);
		}
		...
		if ($category_info) {
			...
			$custom_filter_categories = [27];
			if (in_array($category_id, $custom_filter_categories) || in_array($category_info['parent_id'], $custom_filter_categories) || in_array(27, $parts)) {
				$data['custom_filter'] = true;
			} else {
				$data['custom_filter'] = false;
			}
			
			$data['category_path'] = isset($this->request->get['path']) ? (string)$this->request->get['path'] : '';
