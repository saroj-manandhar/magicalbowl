...
class Cart extends \Opencart\System\Engine\Controller {
	...
	public function getList(): string {
		...
		foreach ($products as $product) {
			$results = $this->model_catalog_product->getRecomended($product['product_id']);
			foreach ($results as $result) {
				$recomended_id = (int)$result['recomended_id'];
				if (!in_array($recomended_id, $cart_product_ids) && !in_array($recomended_id, $recomended_product_ids)) {
					$recomended_product_ids[] = $recomended_id;
				}
			}
		}

		foreach ($recomended_product_ids as $recomended_id) {
			$recomended_info = $this->model_catalog_product->getProduct($recomended_id);

			if ($recomended_info) {
				if ($recomended_info['image']) {
					$image = $this->model_tool_image->resize($recomended_info['image'], $this->config->get('config_image_related_width') ?: 200, $this->config->get('config_image_related_height') ?: 200);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', $this->config->get('config_image_related_width') ?: 200, $this->config->get('config_image_related_height') ?: 200);
				}

				if ($price_status) {
					$price = $this->currency->format($this->tax->calculate($recomended_info['price'], $recomended_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
				} else {
					$price = false;
				}

				if ((float)$recomended_info['special'] && $price_status) {
					$special = $this->currency->format($this->tax->calculate($recomended_info['special'], $recomended_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
				} else {
					$special = false;
				}

				$data['recomended_products'][] = [
					'product_id' => $recomended_info['product_id'],
					'thumb'      => $image,
					'name'       => $recomended_info['name'],
					'price'      => $price,
					'special'    => $special,
					'minimum'    => $recomended_info['minimum'] > 0 ? $recomended_info['minimum'] : 1,
					'href'       => $this->url->link('product/product', 'language=' . $this->config->get('config_language') . '&product_id=' . $recomended_info['product_id'])
				];
			}
		}
