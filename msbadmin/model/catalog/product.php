...
class Product extends \Opencart\System\Engine\Model {
	...
	public function addProduct(array $data): int {
		...
		if (isset($data['product_recomended'])) {
			foreach ($data['product_recomended'] as $recomended_id) {
				$this->model_catalog_product->addRecomended($product_id, $recomended_id);
			}
		}
	...
	public function editProduct(int $product_id, array $data): void {
		...
		// Recomended
		$this->model_catalog_product->deleteRecomended($product_id);
	...
	public function copyProduct(int $product_id): int {
		...
		if ($product_info) {
			...
			$product_data['product_recomended'] = $this->model_catalog_product->getRecomended($product_id);
	...
	public function addVariant(int $master_id, array $data): int {
		...
		if ($master_info) {
			...
			// Recomended
			if (!isset($override['product_recomended'])) {
				$product_data['product_recomended'] = $this->model_catalog_product->getRecomended($master_id);
			}
	...
	public function editVariants(int $master_id, array $data): void {
		...
		foreach ($products as $product) {
			...
			// Recomended
			if (isset($override['product_recomended'])) {
				$product_data['product_recomended'] = $this->model_catalog_product->getRecomended($product['product_id']);
			}
	 ...
	 * Add Recomended
	 *
	 * Create a new recommended record in the database.
	 *
	 * @param int $product_id primary key of the product record
	 * @param int $recomended_id primary key of the product recommended record
	 *
	 * @return void
	 */
	public function addRecomended(int $product_id, int $recomended_id): void {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_recomended` WHERE `product_id` = '" . (int)$recomended_id . "' AND `recomended_id` = '" . (int)$product_id . "'");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "product_recomended` SET `product_id` = '" . (int)$recomended_id . "', `recomended_id` = '" . (int)$product_id . "'");
	}

	/**
	 * Delete Recomended
	 *
	 * Delete recommended record in the database.
	 *
	 * @param int $product_id primary key of the product record
	 *
	 * @return void
	 */
	public function deleteRecomended(int $product_id): void {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_recomended` WHERE `product_id` = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "product_recomended` WHERE `recomended_id` = '" . (int)$product_id . "'");
	}

	/**
	 * Get Recomended
	 *
	 * Get the record of the recommended record in the database.
	 *
	 * @param int $product_id primary key of the product record
	 *
	 * @return array<int, int> recommended records that have product ID
	 */
	public function getRecomended(int $product_id): array {
		$product_recomended_data = [];
