		...
		if (!$this->data) {
			...
			foreach ($cart_query->rows as $cart) {
				...
				if ($product_query->num_rows && ($cart['quantity'] > 0)) {
					...
					$product_options = (array)json_decode((string)$cart['option'], true);

					// Merge variant code with options
					$variant = json_decode((string)$product_query->row['variant'], true);
					...
					if ($cart['override']) {
						$override = json_decode((string)$cart['override']);
					} else {
						$override = [];
					}
